...
router.get("/search/:search_term", async (req, res, next) => {
  try {
    ...
    const filteredDocs = docs.filter(agent => {
      const matchFirstName = agent.firstName
        .toLowerCase()
        .match(search_term.toLowerCase());
      const matchLastName = agent.lastName
        .toLowerCase()
        .match(search_term.toLowerCase());
    ...
    console.error(
      "An error occurred while searching for a specific agent:",
      err
    );
    next(err);
  }
