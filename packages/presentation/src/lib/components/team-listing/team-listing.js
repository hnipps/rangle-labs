    ...
    if (teamMembers.length > 0) {
      team = teamMembers.map((teamMember, i) => {
        ...
        if (this.props.onAddClick) {
          addButton = (
            <Button value={i} color={'green'} onClick={this.props.onAddClick}>
              Add
            </Button>
          )
        }
        if (this.props.onRemoveClick) {
          removeButton = (
            <Button value={i} color={'dark-red'} onClick={this.props.onRemoveClick}>
              Remove
            </Button>
          )
        }
        return (
          <li className="dib ma1 tc h1" key={teamMember._id}>
            ...
            {this.props.renderName && (
              <h2 className="f7 mw3 center mt0 mb2">{`${teamMember.firstName} ${
                teamMember.lastName
              }`}</h2>
            )}
            {addButton}
